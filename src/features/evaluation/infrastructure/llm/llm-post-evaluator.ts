import { z } from 'zod';
import { type PostEvaluator, PostEvaluatorError } from '../../application/post-evaluator';
import {
  CONTENT_TYPES,
...
export class LLMPostEvaluator implements PostEvaluator {
  ...
  async evaluate(input: EvaluatePostInput): Promise<PostJudgments> {
    ...
    try {
      response = await (this.options.fetcher ?? fetch)('https://api.openai.com/v1/responses', {
        method: 'POST',
        headers: {
    ...
    if (response.status === 401 || response.status === 403)
      throw new PostEvaluatorError('authentication', 'LLM evaluator authentication failed.');
    ...
    if (!response.ok) throw new PostEvaluatorError('unavailable', 'LLM evaluator is unavailable.');
    let body: unknown;
    try {
      body = await response.json();
    } catch {
      throw new PostEvaluatorError('invalid-response', 'LLM evaluator returned unreadable JSON.');
    }
    const parsed = responseSchema.safeParse(body);
    if (!parsed.success || parsed.data.status !== 'completed')
      throw new PostEvaluatorError('invalid-response', 'LLM evaluator response shape is invalid.');
    ...
    try {
      ...
      throw new PostEvaluatorError('invalid-response', 'LLM evaluator judgments are not JSON.');
    }
